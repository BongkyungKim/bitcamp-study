  ...
  public void contextInitialized(ServletContextEvent sce) {
    ...
    try {
      ...
      filterConfig.addMappingForServletNames(
          ...
          // forward, include를 넣으면 diapatcher를 다이렉트 실행이아니라 필터를 할 때 꼽아라!
          false, // 요청이 들어올 때 실행하려면 false,
          "DispatcherServlet"); // 특정 서블릿에 필터를 꼽
      ...
      adminFilterConfig.addMappingForUrlPatterns(
          EnumSet.of(DispatcherType.REQUEST, DispatcherType.FORWARD, DispatcherType.INCLUDE), 
          false, 
          "/service/member/*"); // URL에 해당되는 곳에 필터를 꼽
