    ...
    class MyHttpHandler implements HttpHandler {
      ...
      public void handle(HttpExchange exchange) throws IOException {
        ...
        try (StringWriter stringWriter = new StringWriter();
          ...
          if (path.equals("/")) {
            welcomeHandler.service(paraMap, printWriter);

          } else if (path.equals("/board/list")) {
            boardHandler.list(paraMap, printWriter);

          } else if (path.equals("/board/detail")) {
            boardHandler.detail(paraMap, printWriter);

          } else if (path.equals("/board/update")) {
            boardHandler.update(paraMap, printWriter);

          } else if (path.equals("/board/delete")) {
            boardHandler.delete(paraMap, printWriter);

          } else {
            errorHandler.error(paraMap, printWriter);
